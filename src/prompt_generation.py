...
# wxs
def my_generate_prompt_TG_Reasoning(dataset_name, split_name, TK, TG, CK, Q, CoT, A, f_ICL, Q_type=None, mode=None,
                                    eos_token="", f_no_TG=False,
                                    prompt_format='plain'):
    ...
    if f_ICL and mode == 'test':
        ...
        # sp
        # strategy = 'SP'
        file_path = f'../materials/{dataset_name}/prompt_examples_ICL_{strategy}{split_name}{filename_suffix}.txt'
    ...
    if CK is not None:
        common_knowledge = "\n".join(CK)
        prompt += f'\n"Common information":\n{json.dumps(CK)},' if prompt_format.lower() == 'json' else f'\n\nCommon information:\n{common_knowledge}.'
...
def my_generate_prompt_TG_Reasoning_other(dataset_name, split_name, TK, Q, CoT, A, f_ICL, Q_type=None, mode=None,
                                    eos_token="", f_no_TG=False,
                                    prompt_format='plain'):
    assert prompt_format.lower() in ['plain', 'json'], "Prompt format is not recognized."
    ...
    if f_ICL and mode == 'test':
        ...
        file_path = f'../materials/{dataset_name}/prompt_examples_SP.txt'
...
def my_generate_prompt_ICL(dataset_name, split_name, learning_setting, story, Q, C, f_ICL,
                           f_shorten_story, f_using_CoT, Q_type=None):
