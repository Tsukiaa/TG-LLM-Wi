...
class CustomDataCollator(DataCollatorForLanguageModeling):
    def __init__(self, 
                 ...
                 *args,
                 ...
                 max_length,
                 **kwargs):
                        super().__init__(*args, mlm=mlm, **kwargs)  # 假设你不使用 MLM
                        
                        self.max_length = max_length
                        self.instruction_template = instruction_template
                        if isinstance(instruction_template, str):
                            # The user provides a string, must tokenize
                            self.instruction_token_ids = self.tokenizer.encode(self.instruction_template,
...
model_path = "../model/Llama-2-13b-hf"
...
def read_data(dataset_name, prefix, split, f_CoT_bs=False, f_data_aug=False):
    ...
    args:
        dataset_name: string, dataset name
        prefix: string, prefix for the dataset split
    ...
    # prefix = "TGR_"
    file_path = f'../data/{dataset_name}/TGR_{prefix}{split}.json'  # Change this to the correct local path
    ...
    # data_dict = {
    #     'story': [item["story"] for item in data],
    #     'TG': [item["TG"] for item in data],
    #     'question': [item["question"] for item in data],
    #     'answer': [item["answer"] for item in data],
    #     'external knowledge': [item["external knowledge"] for item in data],
    #     'CoT': [item["CoT"] for item in data],
    #     # 'CoT': [CoT_sampling(item["CoT"], 0.5) for item in data],
    #     # 'CoT': [CoT_sampling(item["CoT"], item['CoT_sample_prob']) for item in data],
    #     'candidates': [item["candidates"] for item in data],
    #     'id': [item['id'] for item in data],
    #     'Q-Type': [item['Q-Type'] for item in data]
    ...
    if f_CoT_bs:
        data_dict = {
            'temporal knowledge': [item["temporal knowledge"] for item in data],
            'question': [item["question"] for item in data],
            'answer': [item["answer"] for item in data],
            'common knowledge': [item["common knowledge"] for item in data],
            'CoT': [item["CoT"] for item in data],
            'id': [item['id'] for item in data],
        }
    else:
    ...
    if f_data_aug and dataset_name in ['TGQA']:
        ...
        global_ent_mapping = {}  # Use a global mapping to ensure consistency of entities
        global_names_cnt = {}
        global_time_offset = random.randint(-20, 5)

        extra_data = [rel_entity_dict, global_ent_mapping, global_names_cnt, random_entity_names, global_time_offset]

        data_aug_dict = {'story': [], 'TG': [], 'external knowledge': [], 'question': [], 'CoT': [], 'candidates': [],
                         'answer': [], 'id': [], 'Q-Type': []}
        for sample in dataset:
            TG, EK, Q, CoT, C, A = data_augmentation(dataset_name, sample['TG'], sample['external knowledge'],
                                                     sample['question'], sample['CoT'],
                                                     sample['candidates'], sample['answer'],
                                                     flag_rm_irr_edges=True, flag_change_relations=True,
...
def load_data(args):
    ...
    """
    dataset_selection = ['TGQA', 'TimeQA_easy', 'TimeQA_hard', 'TempReason_l2', 'TempReason_l3', 'MyData'].index(args.dataset)
    dataset_name = ['TGQA', 'TimeQA', 'TimeQA', 'TempReason', 'TempReason', 'MyData'][dataset_selection]
...
def prepare_data(dataset_name, split_name, data_train, data_val, args):
    ...
    def add_prompt(sample):
        ...
        # wxs
        if args.CoT_bs:
            sample['prompt'] = my_generate_prompt_TG_Reasoning(dataset_name, split_name, sample['temporal knowledge'], None,
                                                               sample['common knowledge'], sample['question'].split('?')[0] + '?',
                                                               sample['CoT'], sample['answer'], args.ICL,
                                                               eos_token="</s>", f_no_TG=args.no_TG,
                                                               prompt_format=args.prompt_format)
        else:
...
def print_example_prompts(data, pred_TGs, mode, dataset_name, split_name, args):
    ...
    for i in range(5):
        ...
        if mode == 'train':
            ...
            # wxs
            if args.CoT_bs:
                prompt = my_generate_prompt_TG_Reasoning(dataset_name, split_name, sample['temporal knowledge'], None,
                                                         sample['common knowledge'],
                                                         sample['question'].split('?')[0] + '?', sample['CoT'], sample['answer'],
                                                         args.ICL, mode='train', eos_token="</s>", f_no_TG=args.no_TG,
                                                         prompt_format=args.prompt_format)
            else:
                prompt = my_generate_prompt_TG_Reasoning(dataset_name, split_name, sample['temporal knowledge'], None,
                                                         sample['common knowledge'],
                                                         sample['question'].split('?')[0] + '?', None, sample['answer'],
                                                         args.ICL, mode='train', eos_token="</s>", f_no_TG=args.no_TG,
                                                         prompt_format=args.prompt_format)
        else:
            ...
            # # wxs
            prompt = my_generate_prompt_TG_Reasoning(dataset_name, split_name, sample['temporal knowledge'], pred_TG,
                                                     sample['common knowledge'],
                                                     sample['question'].split('?')[0] + '?', None, None, args.ICL,
                                                     None, mode='test', f_no_TG=args.no_TG,
                                                     prompt_format=args.prompt_format)
...
def train_model(model, tokenizer, data_train, data_val, args, output_dir, max_steps=-1):
    ...
    collator = CustomDataCollator(response_template, tokenizer=tokenizer, max_length=max_seq_length)
...
def test_model(model, tokenizer, data_test, dataset_name, strategy, split_name, pred_TGs, args, batch_size):
    ...
    for i in tqdm(range(len(data_test))):
        ...
        # wxs
        cur_prompt = my_generate_prompt_TG_Reasoning(dataset_name, split_name, sample['temporal knowledge'], pred_TG,
                                                     sample['common knowledge'], sample['question'].split('?')[0] + '?', None,
                                                     None, args.ICL, None, mode='test',
                                                     f_no_TG=args.no_TG, prompt_format=args.prompt_format)
...
def main():
    ...
    if args.print_prompt:
        print_example_prompts(data_train if args.train else data_test, pred_TGs, 'train' if args.train else 'test',
    ...
    if args.test:
        test_model(model, tokenizer, data_test, dataset_name, strategy, split_name, pred_TGs, args, batch_size=8)
